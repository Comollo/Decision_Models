...
Qual è il numero massimo di categorie in ciascun CasinÃ²?
```{r}
...
```
Si!  
Data l'elevata correlazione tra le due variabili, vogliamo inserire un vincolo che tenga conto di questo aspetto. L'assunto è che le categorie con maggior numero di macchine siano anche quelle che ottengono più giocate in un dato mese, ergo sono quelle "preferite" dalla clientela. Se alcune categorie hanno più macchine significa che il manager del Casinò ha constato che in quel periodo i clienti sono particolarmente interessati a quelle categorie.  
Tuttavia per settare l'adeguato numero di macchine in funzione delle giocate occorre anche tenere in considerazione la diversa struttura dei due CasinÃ² (Aries e Libra)
...
Per discretizzare la variabile "giocate totali", abbiamo considerato i quartili della distribuzione di frequenza. Fatta questa premessa, sarebbe stato un errore utilizzare la distribuzione di tale variabile così com'è: come si evince dall'output precedente, in più di un mese, i valori delle giocate totali nei due CasinÃ² sono significativamente diversi. Per non incorrere nell'errore di creare intervalli errati si è deciso che per ciascun mese la variabile "giocate totali" verrà dicretizzata con intervalli differenti a seconda del CasinÃ² di appartenenza.
